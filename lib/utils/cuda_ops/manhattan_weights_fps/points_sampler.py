...
from .manhattan_furthest_point_sample import (mat_weights_furthest_point_sample,
...
class Points_Sampler(nn.Module):
    ...
    Args:
        ...
        weights (list[float]): manhattan weights for fps.
    ...
    def __init__(self,
        ...
        for fps_mod in fps_mod_list:
            ...
            if fps_mod != 'F-FPS':
                self.samplers.append(sampler(weights))
            else:
...
class DFPS_Sampler(nn.Module):
    ...
    def forward(self, points, features, npoint):
        ...
        if self.weights is not None:
            ...
            coors_range = (points.max(1)[0] - points.min(1)[0]).mean(0)
            weights = (coors_range[0][0] / (coors_range + 1e-6)).numpy().tolist()
        fps_idx = mat_weights_furthest_point_sample(points.contiguous(), npoint, weights)
        return fps_idx
...
class FFPS_Sampler(nn.Module):
    ...
    def forward(self, points, features, npoint, manhattan_weights=None):
...
class FS_Sampler(nn.Module):
    ...
    def forward(self, points, features, npoint):
        ...
        fps_idx_dfps = mat_weights_furthest_point_sample(points, npoint, weights)
