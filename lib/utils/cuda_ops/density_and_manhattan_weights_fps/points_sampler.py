...
from .density_and_manhattan_weights_furthest_point_sample import (density_and_manhattan_furthest_point_sample,
...
class PointsSamplerDensityAndManhattan(nn.Module):

    def __init__(self,
                 num_point: List[int],
                 r: List[float],
                 manhattan_weights: List[float],
                 threshold: List[float],
                 fps_mod_list: List[str] = ['D-FPS'],
                 fps_sample_range_list: List[int] = [-1]):
        super(PointsSamplerDensityAndManhattan, self).__init__()
        assert len(num_point) == len(fps_mod_list) == len(fps_mod_list) ==\
            len(manhattan_weights) == len(r)
        self.num_point = num_point
        self.fps_sample_range_list = fps_sample_range_list
        self.samplers = nn.ModuleList()
        for fps_mod, cur_r, cur_mat_weights, cur_threshold in zip(fps_mod_list, r,
                                                                  manhattan_weights, threshold):
            sampler = get_sampler_type(fps_mod)
            if fps_mod != 'F-FPS':
                self.samplers.append(sampler(cur_mat_weights, cur_r, cur_threshold))
            else:
...
class DFPS_Sampler(nn.Module):
    ...
    def __init__(self, manhattan_weights, r, threshold):
    ...
    def forward(self, points, features, npoint):
        ...
        fps_idx = density_and_manhattan_furthest_point_sample(points.contiguous(), npoint, self.r,
                                                              self.manhattan_weights, self.threshold)
        return fps_idx
...
class FS_Sampler(nn.Module):
    ...
    def forward(self, points, features, npoint):
        ...
        fps_idx_dfps = density_and_manhattan_furthest_point_sample(points, npoint, self.r,
                                                                   self.manhattan_weights, self.threshold)
