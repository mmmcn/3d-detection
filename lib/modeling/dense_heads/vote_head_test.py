...
from lib.modeling.builder import build_loss
from lib.modeling.losses import chamfer_distance
from lib.modeling.model_utils import VoteModule
from mmdet3d.ops import build_sa_module, furthest_point_sample
from mmdet.core import build_bbox_coder, multi_apply
from mmdet.models import HEADS
from lib.modeling.dense_heads.base_conv_bbox_head import BaseConvBboxHead


@HEADS.register_module()
class VoteHeadTest(nn.Module):
    ...
    def __init__(self,
                 ...
                 size_res_loss=None,
                 semantic_loss=None):
        super(VoteHeadTest, self).__init__()
    ...
    def forward(self, feat_dict, sample_mod):
        ...
        Args:
            feat_dict (dict): Feature dict from backbone.
                    dict_keys(['sa_xyz', 'sa_features', 'sa_indices'])
                    - sa_xyz: batch * out_channel * 3   [4, 512, 3]
                    - sa_features: batch * (out_channel / 2) * C  [4,256,512]
                    - sa_indices: batch *  out_channel  [4, 512]
        ...
        seed_points, seed_features, seed_indices = self._extract_input(
            feat_dict)
        # (torch.Size([4, 512, 3]), torch.Size([4, 256, 512]), torch.Size([4, 512]))

        # 1. generate vote_points from seed_points
        vote_points, vote_features, vote_offset = self.vote_module(
            seed_points, seed_features)
        # (torch.Size([4, 256, 3]), torch.Size([4, 256, 256]), torch.Size([4, 3, 256]))
        results = dict(
        ...
        # seed_points[4, 512, 3]; seed_features[4,256,512]; vote_points[4,256,3]
        vote_aggregation_ret = self.vote_aggregation(**aggregation_inputs)
        # [4, 256, 3]  [4, 1536, 256], Nonetype
        # aggregated_point还是vote_points
