...
__global__ void density_and_manhattan_weights_furthest_point_sampling_kernel(int b, int n, int m, float threshold,
    ...
    for (int j = 1; j < m; j++) {
        ...
        for (int k = tid; k < n; k += stride) {
            ...
            float d = 0.0;
            if ((x2 * x2 + y2 * y2) <= threshold) {
                d = manhattan_weight_x * (x2 - x1) * (x2 - x1) + manhattan_weight_y * (y2 - y1) * (y2 - y1) +
                manhattan_weight_z * (z2 - z1) * (z2 - z1);
            }
            else {
                d = density_weights[k] * ((x2 - x1) * (x2 - x1) + (y2 - y1) * (y2 - y1) + (z2 - z1) * (z2 - z1));
            }
...
void density_and_manhattan_weights_furthest_point_sampling_kernel_launcher(int b, int n, int m,
    ...
    switch(n_threads) {
        case 1024:
            density_and_manhattan_weights_furthest_point_sampling_kernel<1024>
                <<<b, n_threads, 0, stream>>>(b, n, m, threshold, dataset, manhattan_weights, density_weights, temp, idx);
            break;
        case 512:
            density_and_manhattan_weights_furthest_point_sampling_kernel<512>
                <<<b, n_threads, 0, stream>>>(b, n, m, threshold, dataset, manhattan_weights, density_weights, temp, idx);
            break;
        case 256:
            density_and_manhattan_weights_furthest_point_sampling_kernel<256>
                <<<b, n_threads, 0, stream>>>(b, n, m, threshold, dataset, manhattan_weights, density_weights, temp, idx);
            break;
        case 128:
            density_and_manhattan_weights_furthest_point_sampling_kernel<128>
                <<<b, n_threads, 0, stream>>>(b, n, m, threshold, dataset, manhattan_weights, density_weights, temp, idx);
            break;
        case 64:
            density_and_manhattan_weights_furthest_point_sampling_kernel<64>
                <<<b, n_threads, 0, stream>>>(b, n, m, threshold, dataset, manhattan_weights, density_weights, temp, idx);
            break;
        case 32:
            density_and_manhattan_weights_furthest_point_sampling_kernel<32>
                <<<b, n_threads, 0, stream>>>(b, n, m, threshold, dataset, manhattan_weights, density_weights, temp, idx);
            break;
        case 16:
            density_and_manhattan_weights_furthest_point_sampling_kernel<16>
                <<<b, n_threads, 0, stream>>>(b, n, m, threshold, dataset, manhattan_weights, density_weights, temp, idx);
            break;
        case 8:
            density_and_manhattan_weights_furthest_point_sampling_kernel<8>
                <<<b, n_threads, 0, stream>>>(b, n, m, threshold, dataset, manhattan_weights, density_weights, temp, idx);
            break;
        case 4:
            density_and_manhattan_weights_furthest_point_sampling_kernel<4>
                <<<b, n_threads, 0, stream>>>(b, n, m, threshold, dataset, manhattan_weights, density_weights, temp, idx);
            break;
        case 2:
            density_and_manhattan_weights_furthest_point_sampling_kernel<2>
                <<<b, n_threads, 0, stream>>>(b, n, m, threshold, dataset, manhattan_weights, density_weights, temp, idx);
            break;
        case 1:
            density_and_manhattan_weights_furthest_point_sampling_kernel<1>
                <<<b, n_threads, 0, stream>>>(b, n, m, threshold, dataset, manhattan_weights, density_weights, temp, idx);
            break;
        default:
            density_and_manhattan_weights_furthest_point_sampling_kernel<1024>
