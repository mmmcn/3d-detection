...
import numba
import numpy as np
from .points2voxel import nusc_points_to_voxel_3d_np
import time
...
def points_to_voxel_nusc(cur_sweep_points,
    ...
    """
    if not isinstance(voxel_size, np.ndarray):
        voxel_size = np.array(voxel_size, dtype=cur_sweep_points.dtype)
    if not isinstance(coors_range, np.ndarray):
        coors_range = np.array(coors_range, dtype=cur_sweep_points.dtype)
    voxelmap_shape = (coors_range[3:] - coors_range[:3]) / voxel_size
    # [lx, ly, lz], [500, 30, 500]
    voxelmap_shape = tuple(np.round(voxelmap_shape).astype(np.int32).tolist())
    # don't create large array in jit(nopython=True) code.
    num_points_per_voxel = np.zeros(shape=(max_voxels,), dtype=np.int32)
    sample_points = np.zeros(shape=(max_voxels, cur_sweep_points.shape[-1]), dtype=cur_sweep_points.dtype)
    coor_to_voxelidx = -np.ones(shape=voxelmap_shape, dtype=np.int32)
    voxels = np.zeros(
    ...
    voxel_num = nusc_points_to_voxel_3d_np(
        cur_sweep_points, other_sweep_points, voxels, sample_points,
        coors,
        num_points_per_voxel, coor_to_voxelidx, voxel_size.tolist(),
        coors_range.tolist(), max_points, max_voxels, cur_sample_num, seed)
    ...
    # return voxels, num_points_per_voxel
    return voxels, num_points_per_voxel, sample_points
