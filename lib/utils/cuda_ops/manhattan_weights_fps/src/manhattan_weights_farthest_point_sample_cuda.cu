...
__global__ void manhattan_weights_furthest_point_sampling_kernel(
  ...
  for (int j = 1; j < m; j++) {
    ...
    for (int k = tid; k < n; k += stride) {
      ...
      float d =
          weight_x * (x2 - x1) * (x2 - x1) + weight_y * (y2 - y1) * (y2 - y1) +
          weight_z * (z2 - z1) * (z2 - z1);
...
void manhattan_weights_furthest_point_sampling_kernel_launcher(int b, int n, int m,
                                                               const float *dataset,
                                                               const float *weights,
  ...
  switch (n_threads) {
        case 1024:
            manhattan_weights_furthest_point_sampling_kernel<1024>
                <<<b, n_threads, 0, stream>>>(b, n, m, weights, dataset, temp, idxs);
            break;
        case 512:
            manhattan_weights_furthest_point_sampling_kernel<512>
                <<<b, n_threads, 0, stream>>>(b, n, m, weights, dataset, temp, idxs);
            break;
        case 256:
            manhattan_weights_furthest_point_sampling_kernel<256>
                <<<b, n_threads, 0, stream>>>(b, n, m, weights, dataset, temp, idxs);
            break;
        case 128:
            manhattan_weights_furthest_point_sampling_kernel<128>
                <<<b, n_threads, 0, stream>>>(b, n, m, weights, dataset, temp, idxs);
            break;
        case 64:
            manhattan_weights_furthest_point_sampling_kernel<64>
                <<<b, n_threads, 0, stream>>>(b, n, m, weights, dataset, temp, idxs);
            break;
        case 32:
            manhattan_weights_furthest_point_sampling_kernel<32>
                <<<b, n_threads, 0, stream>>>(b, n, m, weights, dataset, temp, idxs);
            break;
        case 16:
            manhattan_weights_furthest_point_sampling_kernel<16>
                <<<b, n_threads, 0, stream>>>(b, n, m, weights, dataset, temp, idxs);
            break;
        case 8:
            manhattan_weights_furthest_point_sampling_kernel<8>
                <<<b, n_threads, 0, stream>>>(b, n, m, weights, dataset, temp, idxs);
            break;
        case 4:
            manhattan_weights_furthest_point_sampling_kernel<4>
                <<<b, n_threads, 0, stream>>>(b, n, m, weights, dataset, temp, idxs);
            break;
        case 2:
            manhattan_weights_furthest_point_sampling_kernel<2>
                <<<b, n_threads, 0, stream>>>(b, n, m, weights, dataset, temp, idxs);
            break;
        case 1:
            manhattan_weights_furthest_point_sampling_kernel<1>
                <<<b, n_threads, 0, stream>>>(b, n, m, weights, dataset, temp, idxs);
            break;
        default:
            manhattan_weights_furthest_point_sampling_kernel<512>
