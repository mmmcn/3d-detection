...
__global__ void density_weights_furthest_point_sampling_kernel(int b, int n, int m,
    ...
    for (int j = 1; j < m; j++) {
        ...
        for (int k = tid; k < n; k += stride) {
            ...
            float d = weights[k] * ((x2 - x1) * (x2 - x1) + (y2 - y1) * (y2 - y1) + (z2 - z1) * (z2 - z1));
...
void density_weights_furthest_point_sampling_kernel_launcher(int b, int n, int m,
    ...
    switch(n_threads) {
        case 1024:
            density_weights_furthest_point_sampling_kernel<1024>
                <<<b, n_threads, 0, stream>>>(b, n, m, dataset, weights, temp, idx);
            break;
        case 512:
            density_weights_furthest_point_sampling_kernel<512>
                <<<b, n_threads, 0, stream>>>(b, n, m, dataset, weights, temp, idx);
            break;
        case 256:
            density_weights_furthest_point_sampling_kernel<256>
                <<<b, n_threads, 0, stream>>>(b, n, m, dataset, weights, temp, idx);
            break;
        case 128:
            density_weights_furthest_point_sampling_kernel<128>
                <<<b, n_threads, 0, stream>>>(b, n, m, dataset, weights, temp, idx);
            break;
        case 64:
            density_weights_furthest_point_sampling_kernel<64>
                <<<b, n_threads, 0, stream>>>(b, n, m, dataset, weights, temp, idx);
            break;
        case 32:
            density_weights_furthest_point_sampling_kernel<32>
                <<<b, n_threads, 0, stream>>>(b, n, m, dataset, weights, temp, idx);
            break;
        case 16:
            density_weights_furthest_point_sampling_kernel<16>
                <<<b, n_threads, 0, stream>>>(b, n, m, dataset, weights, temp, idx);
            break;
        case 8:
            density_weights_furthest_point_sampling_kernel<8>
                <<<b, n_threads, 0, stream>>>(b, n, m, dataset, weights, temp, idx);
            break;
        case 4:
            density_weights_furthest_point_sampling_kernel<4>
                <<<b, n_threads, 0, stream>>>(b, n, m, dataset, weights, temp, idx);
            break;
        case 2:
            density_weights_furthest_point_sampling_kernel<2>
                <<<b, n_threads, 0, stream>>>(b, n, m, dataset, weights, temp, idx);
            break;
        case 1:
            density_weights_furthest_point_sampling_kernel<1>
                <<<b, n_threads, 0, stream>>>(b, n, m, dataset, weights, temp, idx);
            break;
        default:
            density_weights_furthest_point_sampling_kernel<1024>
